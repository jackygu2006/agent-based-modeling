...
to go
  if ticks > 5000 or current-x-lp <= 0 or current-y-lp <= 0 [stop]
  update-price (random 100 < buy-possibility) (random max-trade-amount + 100)
  if price > max-price [set max-price price]
  if price < min-price [set min-price price]
  tick
end
...
true
false
"" "set-plot-y-range min-price max-price"
PENS
"default" 1.0 0 -15040220 true "" "plot price"
