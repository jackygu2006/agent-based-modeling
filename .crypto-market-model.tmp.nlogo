...
globals [
  ;;Set by slider or input
  ;;initial-traders                ;;initial traders, not total traders
  ;;max-traders
  ;;ico-distribute-ratio           ;;if allocate to some participants as private or public sale, set the ratio of this amount
  ...
  max-ticks                        ;;max emulation days
  total-amount                     ;;total amount of tokens
  initial-price                    ;;set the initialized exchange rate of token A to token B is 1
  last-price                       ;;last exchange rate
  ...
  current-x-liquidity              ;;current liquidity of token A
  current-y-liquidity              ;;current liquidity of token B
  max-price                        ;;max price(exchange rate) of all period
  min-price                        ;;min price of all period

  ;;ema - exponential moving average index, see: https://www.steema.com/docs/financialFunctionsRef/expMovingAverageFunction.htm
  alpha1                           ;;alpha, smoothing factor = 2 / (1 + N); N - number of time periods
  ema-price1                       ;;ema = (current-price * alpha) + current-ema * (1 - alpha)
  alpha2                           ;;here I use 2 group of ema
  ema-price2
]

;;define traders(turtles)'s properties
turtles-own [
  %influence                       ;;influence power, to cover the circle of patches in radius
  %independent-mind                ;;independent mind, 0-100
  is-ico                           ;;if participate the private or public sale
  is-market-maker                  ;;if the trader is market maker, which is only buy, never sell
  amount                           ;;token A amount. In this model, the traders buy with his all amount of Token B(USD) and sell all amount of token A
  price                            ;;the exchange rate of token B(USD) to token A, which is buying price. ico rate is 1
  buy-ticks                        ;;the ticks(time) of buy token A
  initial-cash                     ;;initial amount of token B(USD), also consider as cash
  cash                             ;;amount of token B which trader can spend
  %stop-limit                      ;;stop limit rate. If the current price increasement more than this rate, sell all.
  %stop-loss                       ;;stop loss rate. If the current price decreasing more than this rate, sell all
  max-trade-times                  ;;max trade times, if touch this, the trader will not trade any more
  trade-times                      ;;total trading times
  checked-today                    ;;if the trading status was checked today
  max-hold-days                    ;;the max days traders will keep the token A in hands. After this period, if the price will has not touched the
                                   ;;stop-loss or stop-limit, sell all on the current price.
  total-assets                     ;;value of token B + value of token A
  %asset-change                    ;;total-assets changed from the initial amount of tokenB(USD)
]
...
to initialize-params
  ...
  set ema-price1 1
  set ema-price2 1
  set alpha1 2 / (1 + ema1-days)
  set alpha2 2 / (1 + ema2-days)

  ;;uniswap
  set current-x-liquidity initial-liquidity
  set current-y-liquidity initial-liquidity
  set max-price 0
  set min-price 999999
  set current-price current-y-liquidity / current-x-liquidity
end

to set-turtle [_who _isbuy _around-kol _initial]
  ask turtle _who [
    ifelse _around-kol
      [setxy xcor + (random 20 - 10) ycor + (random 20 - 10)] ;;set turtles near the kol
      [setxy random-xcor random-ycor]                         ;;set turtles randomly
    set shape "person"
    set color gray + 2
    set size 3
;    set label-color black
    ...
    ;;set is-ico
    ifelse _initial [
      set is-ico true                                       ;;if allocated at the begining
    ] [
      set is-ico false                                      ;;if affected and created buy the kol
    ]

    ;;set affecting range according to %influence, for debuging
    ...
    set cash round (random-exponential 10) * 500 * new-cash-x
    set initial-cash cash
    ...
    ifelse _isbuy [
      ...
      set amount floor (distribute-amount / initial-traders)   ;;allocate ico amount averagely
...
to go
  if ticks >= max-ticks or current-price <= 0 [stop]

  ask turtles [set checked-today false]

  ask turtles [
    ;;move
    let speed 1
    if %influence > 3 [set speed %influence / 3]                              ;;more influence, more fast moving, divided 2 to slowdown
...
;    if ticks mod 10 = 0 [set %influence %influence * (1 + %asset-change / 1000)] ;;根据他的总盈利调整影响力
  ...
  tick
  ;; set ema-price
  set ema-price1 (current-price - ema-price1) * alpha1 + ema-price1 ;;green color
  set ema-price2 (current-price - ema-price2) * alpha2 + ema-price2 ;;orange color
end
...
to affect-new [kol-turtle]
  ask turtle kol-turtle [
    if random 100 < bullish-index * 2 [    ;;possibility to affect new guys depends on bullish-index (0~20%)
      hatch %influence [set-turtle who false true false]
    ]
  ]
end
...
to check-around [kol-turtle]
  ask turtle kol-turtle [
    ...
    ;;2- for traders affected by kol
    let kol-amount amount
    ask other turtles in-radius %influence [
      if not checked-today [
        ifelse trade-strategy1 false who [sell who]
        [
          ifelse random 100 < %independent-mind [
            ...
            ;;affect by the kol's position
            if kol-amount > 0 and amount = 0 [buy who]
            if kol-amount = 0 and amount > 0 [
              let pre-price price
              sell who
            ]
          ]
...
;;get price according to uniswap AMM - unlimited lp
...
;;update price according to uniswap AMM - limited lp
to update-price2 [is-buy trade-amount]
  set last-price current-price
  ifelse is-buy [
    let y-amount current-y-liquidity * trade-amount / (current-x-liquidity - trade-amount)
    set current-x-liquidity current-x-liquidity - trade-amount
    set current-y-liquidity current-y-liquidity + y-amount
    let _price y-amount / trade-amount
    set current-price round (_price * 10 ^ 3) / 10 ^ 3   ;;decimal is 3
  ] [
    let y-amount current-y-liquidity * trade-amount / (current-x-liquidity + trade-amount)
    set current-x-liquidity current-x-liquidity + trade-amount
    set current-y-liquidity current-y-liquidity - y-amount
  ...
  if current-price > max-price [set max-price price]
  if current-price < min-price [set min-price price]
end
...
to-report trade-strategy1 [check-buy trader-id]
  ...
  [
    ...
    ask turtle trader-id [
      set re amount > 0 and
        ((random 100 < %sell-on-stop-limit and current-price > price * (1 + %stop-limit / 100))
...
to-report trade-strategy2 [check-buy trader-id]
  ;;TODO
  report true
end
...
to show-params
  ...
  show (word "max-hold-days ->     " (max [max-hold-days] of turtles) " " (min [max-hold-days] of turtles))
end

;;testing for random price according to bullish index
to random-price
  update-price2 (random 10 < bullish-index) (random (10000 - 500) + 500)
end
...
"ema1" 1.0 0 -11085214 true "" "plot ema-price1"
"ema2" 1.0 0 -955883 true "" "plot ema-price2"
